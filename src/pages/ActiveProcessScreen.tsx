...
import { ChevronRight, CheckCircle, AlertTriangle, BatteryWarning } from 'lucide-react';
...
export default function ActiveProcessScreen() {
  ...
  // Track last agitation beep to prevent rapid-fire loops
  ...
  useEffect(() => {
    if (status === 'RUNNING' && endTimeMs !== null) {
      const updateTimer = () => {
        ...
        // ---------------------------------------------------------
        // Agitation Check Logic (Absolute calculation)
        if (recipe && currentStepIndex < recipe.steps.length) {
          ...
          if (step.agitation_interval_sec > 0) {
            ...
            if (isAgitationTick && lastAgitationSecRef.current !== elapsedSec) {
              ...
              beep(600, 'square'); // Short agitation tone
            }
          }
        }
        // ---------------------------------------------------------
        ...
        if (diff > 0) {
          ...
          beep(400, 'sine', 1.5); // Long step complete tone
  ...
  useEffect(() => {
    // Release wake lock when unmounting entirely
    return () => {
  ...
  const handleStartProcess = async () => {
    // Unlock Audio inside user gesture
  ...
  if (!recipe) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center text-[var(--text-secondary)] h-full gap-4">
        ...
        <h2 className="text-xl font-bold text-[var(--text-primary)]">No Active Session</h2>
        <p>Return to the main screen to configure and select a recipe to start.</p>
        <button className="btn btn-secondary mt-4 w-full" onClick={() => navigate('/')}>Go Root</button>
      </div>
    );
  }
    ...
    return (
      <div className="p-4 flex flex-col h-full gap-4 pb-20 overflow-y-auto">
        <div className="flex items-center justify-between border-b border-[var(--border)] pb-4">
          <h2 className="text-2xl font-bold tracking-tighter text-[var(--text-primary)]">PRE-CHECK</h2>
        ...
        <div className="flex flex-col gap-3 my-4 flex-1 justify-center">
          <label className="card flex items-center justify-between cursor-pointer group hover:border-[var(--accent)] transition-all">
            <span className="font-medium text-lg text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]">Developer Temp ~ {recipe.base_temp_c}°C</span>
          ...
          <label className="card flex items-center justify-between cursor-pointer group hover:border-[var(--accent)] transition-all">
            <span className="font-medium text-lg text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]">Film Loaded & Tank Sealed</span>
            <input type="checkbox" className="w-8 h-8 rounded accent-[var(--success)]" 
          ...
          <label className="card flex items-center justify-between cursor-pointer group hover:border-[var(--accent)] transition-all">
            <span className="font-medium text-lg flex items-center gap-2 text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]">
              <BatteryWarning className="text-[var(--danger)] shrink-0" size={20} /> Ensure Not Silent Mode
            </span>
            <input type="checkbox" className="w-8 h-8 rounded accent-[var(--success)]" 
  ...
  if (status === 'FINISHED') {
    return (
      <div className="flex items-center justify-center flex-col p-8 h-full gap-4 text-center">
        ...
        <h2 className="text-3xl font-black mt-4">Process Finished</h2>
        <p className="opacity-75">Safely open the tank.</p>
        <button className="btn btn-secondary mt-8 w-full shadow-lg h-14" onClick={() => { reset(); navigate('/'); }}>Complete & Save Log</button>
      </div>
    );
  }
    ...
    <div className="relative flex flex-col h-full bg-[var(--bg-primary)] select-none">
      ...
      <div className="flex flex-col flex-1 pb-16 justify-between text-center relative pointer-events-none">
        <div className="pt-6">
          ...
          <p className="text-[var(--text-secondary)] mt-2 font-mono uppercase text-sm tracking-widest">Step {currentStepIndex + 1} of {recipe.steps.length}</p>
        </div>

        {/* Huge Full Width Monospace Timer Display */}
        <div className="w-full relative flex items-center justify-center py-8 bg-[var(--bg-secondary)] border-y border-[var(--border)] font-mono drop-shadow-2xl opacity-100 transition-opacity">
          <span className="text-[7rem] leading-none font-black tracking-tighter text-[var(--text-primary)]">
            {mins}:{secs}
          </span>
        </div>

        {/* Agitation UI Indicator */}
        <div className="flex flex-col items-center justify-center p-4">
          <div className="text-sm border border-[var(--border)] px-4 py-3 rounded-xl bg-[var(--bg-secondary)] inline-flex flex-col gap-1 items-center shadow-inner mt-4 pointer-events-auto">
             <span className="opacity-70 font-semibold uppercase tracking-wider text-[10px] text-[var(--text-secondary)]">Instruction</span>
        ...
        <div className="border-t border-[var(--border)] w-full py-4 bg-[var(--bg-secondary)]/50 pointer-events-auto mt-auto">
          ...
          {upcomingSteps.length > 0 ? (
            ...
            <div className="px-4 text-left mt-2 block w-full">
              <span className="opacity-50 text-sm">Final Stage</span>
            </div>
          )}
        </div>
      </div>
