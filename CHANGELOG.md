...
## [v1.1.0] - 2026-03-23
### ✨ 기능 추가 (Features)
- **위키(Wiki) 고도화 (동적 온도 계산기):** 아레니우스 방식을 차용한 `t2 = t1 * exp(0.081 * (T1 - T2))` 기반의 동적 온도-시간 보상(Compensation) 계산기(Calculator) 모듈을 위키 페이지 내부에 삽입하여, 기준 온도와 실측 온도가 다를 때 소모해야 할 초(sec)를 자동 변환해줌.
